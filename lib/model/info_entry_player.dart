...
class InfoEntryPlayerBean {
  ...
  String toString() {
    ...
    return "${getNomPrise(infoEntry.prise)} de $player${withPlayers != null && (withPlayers?.length ?? 0) > 0 ? " (avec ${withPlayers![0]}${withPlayers?.length == 2 ? " et ${withPlayers![1]}" : ""}" : ""}, ${infoEntry.points} points pour $campDesPoints, ${infoEntry.nbBouts} bout(s) pour $campDesPoints.";
  }
}
