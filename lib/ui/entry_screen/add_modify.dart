...
class AddModifyEntry extends HookConsumerWidget {
  ...
  String routeNameParam(int roundId) => "$routeName/$roundId";
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return ref.watch(currentInfoEntryProvider).when(
          error: (error, stack) => Text(error.toString()),
          loading: () => const Center(child: CircularProgressIndicator()),
          data: (data) {
            if (data != null) {
              entry.value = data.infoEntry;
            }
            final List<Player>? playersArg =
                ref.watch(currentPlayersProvider).value;
            if (playersArg == null) {
              //FIXME to change with a viewModel instead of dealing with this in the ui
            ...
            final args = useMemoized(() => ModalRoute.of(context)
                ?.settings
                .arguments as AddModifyArguments?);
            ...
            useEffect(() {
              ...
              if (info == null) {
                ...
                if (pLength == 5) {
                  ...
                  info = info.copyWith(
                      withPlayers: [playersValue[0], playersValue[0]]);
                }
              }
              playerAttack.value = info.player;
              entry.value = info.infoEntry;
              withPlayers.value = info.withPlayers;
              textPointsController.text =
                  info.infoEntry.points.toStringAsFixed(1);
              return null;
            }, [0]);
            return BackgroundGradient(
              child: Scaffold(
                appBar: AppBar(
                  title: Text(S
                      .of(context)
                      .addModifyAppBarTitle(add.value ? 'add' : 'modify')),
                ),
                floatingActionButton: FloatingActionButton(
                    ...
                    onPressed: () {
                      if (_validate(
                        ...
                        if (data != null) {
                          ref.read(entriesProvider)?.inModifyEntry.add(
                              InfoEntryPlayerBean(
                                  player: playerAttack.value,
                                  infoEntry: entry.value,
                                  withPlayers: withPlayers.value));
                        } else {
                          ref.read(entriesProvider)?.inAddEntry.add(
                              InfoEntryPlayerBean(
                                  player: playerAttack.value,
                                  infoEntry: entry.value,
                                  withPlayers: withPlayers.value));
                        }
                        context.goNamed(TableauPage.routeName);
                      } else {
                                ...
                                Row(
                                  ...
                                  children: <Widget>[
                                    ...
                                    Padding(
                                      ...
                                      child: Text(S.of(context).addModifyOudler(
                                          entry.value.nbBouts)),
                                    )
                                  ],
                                )
                            ...
                            child: Column(children: <Widget>[
                              ...
                              Row(
                                ...
                                children: <Widget>[
                                  Checkbox(
                                      value:
                                          entry.value.petitsAuBout.isNotEmpty &&
                                              entry.value.petitsAuBout[0] !=
                                                  Camp.none,
                                      onChanged: (bool? value) {
                                        if (value != null) {
                                          var p =
